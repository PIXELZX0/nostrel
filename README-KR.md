...
- [x] **NIP-17** 비공개 DM *(조건부: 외부에서 오는 DM 수용, 기본 켜짐)*
...
- [x] **NIP-59** gift wrap — 열람 제한을 릴레이가 강제하고, 고객 앞으로 온 것은 수용 *(조건부: 기본 켜짐)*
- [x] **NIP-62** 잊혀질 권리
- [x] **NIP-70** 보호 이벤트
...
| 17 | 비공개 DM | NIP-59 gift wrap 위에서 동작. 고객 앞으로 온 wrap은 수용하고 그 고객에게 청구 |
...
| 59 | gift wrap | kind 1059·13은 발신자·수신자만 조회, 백데이트 허용. 일회용 서명 키에는 계정이 필요 없음 |
...
NIP-11 `supported_nips`는 **실제 상태를 그대로** 내보낸다. 항상 켜져 있는 것(01·04·09·11·40·42·45·50·56·62·70·77·86·98)은 고정이고, 나머지는 설정에 따라 붙었다 떨어진다:
...
| 05 | 판매 중인 도메인이 하나 이상 |
| 17 / 59 | **외부에서 오는 DM 허용** (기본 켜짐) |
| 57 / 58 | 각 제3자 이벤트 토글 |
| 96 | 파일 저장소 사용 가능 |

04·17·59가 여기 있는 이유는 단순히 저장해서가 아니라, **누가 읽을 수 있는지를 릴레이가 강제**하기 때문이다. 17·59는 여기에 더해 누가 고객에게 쓸 수 있는지도 강제한다.

### 제3자가 고객 앞으로 쓰는 이벤트 (NIP-04 · NIP-17 · NIP-57 · NIP-58)

이것들만 예외다. 작성자가 고객이 아닌데 **고객에 관한** 이벤트이거나 **고객 앞으로 온** 이벤트라, 그대로 두면 전부 거부된다.

| 이벤트 | 작성자 | 설정 |
|---|---|---|
| gift wrap (kind 1059) · kind 4 DM | 일회용 키(NIP-59) 또는 계정 없는 발신자 | **외부에서 오는 DM 허용** *(켜짐)* |
| zap 영수증 (kind 9735) | 결제를 받은 LNURL 서버 | **제3자 zap 영수증 허용** |
| 배지 수여 (kind 8) | 배지 발행자 | **제3자 배지 수여 허용** |

켜면 다음을 만족할 때만 통과한다.

- **공통**: 후보 pubkey 중 이 릴레이에 계정(또는 그룹)이 있는 **첫 번째** 사람이 지불자가 되고, 그 사람의 쓰기 권한·용량을 통과해야 한다. 후보가 하나도 고객이 아니면 거부.
- **DM**: `p` 태그는 정확히 하나, `content`는 비어 있지 않을 것. 후보 목록이 **작성자**부터 시작하므로 고객이 보내는 메시지는 본인 용량에서 나가고, 외부에서 온 것만 수신자에게 청구된다. 토글도 이 경우에만 적용된다.
...
용량은 **수신자에게** 청구된다(올린 쪽이 아니라). 결제나 수여가 실제로 일어났는지는 릴레이가 알 수 없으므로, 켜는 순간 모르는 사람이 고객의 용량을 쓸 수 있다는 뜻이다 — 그래서 zap·배지는 기본값이 꺼짐이고, 악용하는 작성자는 NIP-86으로 밴한다. 켜져 있을 때만 NIP-11 `supported_nips`에 각각 57·58이 올라간다.

**DM만 기본값이 켜짐이다.** NIP-59 gift wrap은 *항상* 그 메시지 하나만을 위한 키로 서명되므로, 끄면 NIP-17이 아예 동작하지 않는다 — 고객끼리도 마찬가지다. 그래서 끄면 `supported_nips`에서 17·59도 함께 빠진다. 켜 두는 대가는 아무나 고객에게 메시지를 보내 그 용량을 쓸 수 있다는 것이다. NIP-86으로 밴하거나, 꺼서 kind 4를 고객끼리만 주고받게 한다.
...
wrap을 서명한 키는 메시지 하나를 보내고 버려지므로 고객이 될 수 없다. 그래서 `p` 태그 하나가 고객을 가리키는 wrap은 그 고객 몫으로 수용하고 그 고객에게 청구한다 — [제3자가 고객 앞으로 쓰는 이벤트](#제3자가-고객-앞으로-쓰는-이벤트-nip-04--nip-17--nip-57--nip-58) 참고.
...
패널이 소유하는 값(`관리자`): 릴레이 이름·설명·아이콘·배너, 운영자 연락처·pubkey, 테마, 보관 기간, 국가·언어·주제, 요금, NIP-05 할증, NIP-46 relay, nsite 도메인, 자동 초대, kind 정책, 외부 DM 허용, 제3자 zap·배지 허용, `read_auth_required`, `min_pow`, `created_at` 허용치, 결제 백엔드와 자격증명, 미디어 백엔드와 `max_blob_size_mb`.
