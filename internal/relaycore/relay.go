...
func NewRelay() *Relay {
	...
	rl := &Relay{
		...
		Info: &nip11.RelayInformationDocument{
			Software:      "https://nostrel/internal/relaycore",
			Version:       "n/a",
...
type Relay struct {
	...
	// OverwriteOK rewrites the OK message sent back for an event, after the
	// relay has decided its fate. It is the only way to control the wording of
	// a *successful* response: ephemeral events otherwise always report
	// "broadcasted to N listeners", which NIP-43 needs to replace with its own
	// welcome and duplicate messages.
	//
	// FORK: not in upstream khatru.
	OverwriteOK      []func(ctx context.Context, event *nostr.Event, ok bool, reason string) (bool, string)
