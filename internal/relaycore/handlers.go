		...
		for {
			...
			go func(message string) {
				...
				case *nostr.EventEnvelope:
					...
					// FORK: let the application have the last word on the OK
					// message. See Relay.OverwriteOK.
					for _, ovw := range srl.OverwriteOK {
						ok, reason = ovw(ctx, &env.Event, ok, reason)
					}
				...
				case *nostr.ReqEnvelope:
					...
					reqCtx, cancelReqCtx := newReqContext(ctx, env.SubscriptionID)
...
// newReqContext builds the context one REQ's stored-event lookups run under.
//
// FORK: inlined upstream. The cancel func is handed to the listener registry
// and called when the subscription ends, but `go vet` cannot see that from the
// message switch and reports a lost cancel. Returning it from a helper states
// the ownership transfer in a way the check understands; behaviour is
// unchanged.
func newReqContext(ctx context.Context, subscriptionID string) (context.Context, context.CancelCauseFunc) {
	reqCtx, cancel := context.WithCancelCause(ctx)
	return context.WithValue(reqCtx, subscriptionIdKey, subscriptionID), cancel
}
